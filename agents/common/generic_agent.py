...
class GenericAgent():
    ...
    def __init__(self):
        ...
        # Set the chunk size to the configured maximum number of files that can be processed in a
        # single action, but absolutely at most 5000 if the maxium is defined to be larger than that.
        self._chunk_size = min(self._uida_conf_vars.get('MAX_FILE_COUNT', 5000), 5000)
    ...
    def _get_action_record(self, message):
        ...
        if response.status_code == 404:
            ...
            raise Exception(
                'IDA api returned an error when trying to retrieve action record. Code: %d. Error: %s' % (response.status_code, response.text)
            )
        ...
        try:
            ...
            self._logger.exception(
                'Could not get json from http response - instead response has the following content: %s' % response.text
            )
            raise
    ...
    def _get_nodes_associated_with_action(self, action):
        ...
        if response.status_code != 200:
            raise Exception(
                'IDA api returned an error. Code: %d. Error: %s' % (response.status_code, response.text)
            )
        nodes = response.json()
        ...
        for node in nodes:
            ...
            if update_node:
                ...
                if response.status_code not in (200, 201, 204):
                    ...
                    raise Exception(error_msg % (node['pid'], str(response.text)))
    ...
    def _http_request(self, method, url, data=None, headers=None):
        ...
        for i in range(1, retry_policy['max_retries'] + 1):
            try:
                ...
                self._logger.debug('Response: %d %s' % (response.status_code, response.text))
