...
class TestAuditing(unittest.TestCase):
    ...
    def test_auditing(self):
        ...
        self.assertEqual(report_data.get("filesystemNodeCount"), 127)
        self.assertEqual(report_data.get("nextcloudNodeCount"), 127)
        self.assertEqual(report_data.get("frozenFileCount"), 0)
        self.assertEqual(report_data.get("metaxFileCount"), 0)
        self.assertEqual(report_data.get("invalidNodeCount"), 0)
        self.assertEqual(report_data.get("errorNodeCount"), 0)
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/list-active-projects %s" % (
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/audit-active-projects %s" % (
            self.config["HTTPD_USER"],
            self.config["ROOT"],
            self.config["START"]
        )
        ...
        self.assertEqual(report_data.get("filesystemNodeCount"), 117)

        print("Verify correct number of reported Nextcloud nodes")
        self.assertEqual(report_data.get("nextcloudNodeCount"), 129)
        ...
        print("Verify correct number of reported filesystem nodes")
        self.assertEqual(report_data.get("filesystemNodeCount"), 129)
        ...
        print("Verify correct number of reported filesystem nodes")
        self.assertEqual(report_data.get("filesystemNodeCount"), 126)
        ...
        print("Verify correct number of reported filesystem nodes")
        self.assertEqual(report_data.get("filesystemNodeCount"), 130)

        print("Verify correct number of reported Nextcloud nodes")
        self.assertEqual(report_data.get("nextcloudNodeCount"), 130)
        ...
        print("Verify correct number of reported Nextcloud nodes")
        self.assertEqual(report_data.get("nextcloudNodeCount"), 117)
        ...
        print("Verify correct number of reported filesystem nodes")
        self.assertEqual(report_data.get("filesystemNodeCount"), 122)

        print("Verify correct number of reported Nextcloud nodes")
        self.assertEqual(report_data.get("nextcloudNodeCount"), 122)
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/check-datasets-pre-repair %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_a)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/fetch-dataset-in-metax %s" % (self.config["HTTPD_USER"], self.config["ROOT"], dataset_pid)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s --force" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_a)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_b)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_c)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_d)
        ...
        print("Verify correct number of reported Nextcloud nodes")
        self.assertEqual(report_data.get("nextcloudNodeCount"), 126)
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project test_project_d" % (self.config["HTTPD_USER"], self.config["ROOT"])
        failed = False
        try:
            output = subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT).decode(sys.stdout.encoding)
        except subprocess.CalledProcessError as error:
        ...
        print("(repairing project D)")
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project test_project_d --force" % (self.config["HTTPD_USER"], self.config["ROOT"])
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/mark-action-as-cleared test_project_a %s" % (self.config["HTTPD_USER"], self.config["ROOT"], action['pid'])
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        #print("Metax frozen file PIDs 2: %s" % json.dumps(metax_file_pids_2))
        #print("Metax frozen file PID count 2: %d" % len(metax_file_pids_2))
        self.assertEqual(len(frozen_file_pids_2), 95)
        self.assertEqual(len(metax_file_pids_2), 95)
        ...
        #print("Metax frozen file diff 2v1: %s" % json.dumps(metax_file_pid_diff_2v1))
        self.assertEqual(len(frozen_file_pid_diff_1v2), 0)
        self.assertEqual(len(frozen_file_pid_diff_2v1), 95)
        self.assertEqual(len(metax_file_pid_diff_1v2), 0)
        self.assertEqual(len(metax_file_pid_diff_2v1), 95)
        ...
        #print("Metax frozen file PIDs 3: %s" % json.dumps(metax_file_pids_3))
        #print("Metax frozen file PID count 3: %d" % len(metax_file_pids_3))
        self.assertEqual(len(frozen_file_pids_3), 68)
        self.assertEqual(len(metax_file_pids_3), 95)
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project test_project_e --force" % (self.config["HTTPD_USER"], self.config["ROOT"])
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
        ...
        self.assertEqual(len(frozen_file_pids_4), 68)
        self.assertEqual(len(metax_file_pids_4), 68)
        ...
        self.assertEqual(len(frozen_file_pids_5), 95)
        self.assertEqual(len(metax_file_pids_5), 95)
        ...
        self.assertEqual(len(frozen_file_pids_6), 68)
        self.assertEqual(len(metax_file_pids_6), 95)
        ...
        print("Verify correct number of reported IDA frozen files")
        self.assertEqual(report_data.get("frozenFileCount"), 68)

        print("Verify correct number of reported Metax files")
        self.assertEqual(report_data.get("metaxFileCount"), 95)
        ...
        cmd = "sudo -u %s DEBUG=false %s/appsupport/repair-project %s" % (self.config["HTTPD_USER"], self.config["ROOT"], report_pathname_e)
        try:
            output = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True).decode(sys.stdout.encoding).strip()
        except subprocess.CalledProcessError as error:
            self.fail(error.output.decode(sys.stdout.encoding))
