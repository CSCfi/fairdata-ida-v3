...
Assuming IDA has been installed to /usr/local/fd/fairdata-ida-v3:

Before proceeding, ensure the rabbitmq management plugin is installed, and the
config file is in place in /usr/local/fd/fairdata-ida-v3/config/config.sh and contains valid parameters
for whatever environment you are running in. The utility script
rabbitmq_create_users in /usr/local/fd/fairdata-ida-v3/utils should be executed to set the admin user
...
/usr/local/fd/fairdata-ida-v3/utils/initialize_venv

# enable rabbitmq management plugin, initialize and configure rabbitmq users, exchanges, and queues
/usr/local/fd/fairdata-ida-v3/utils/initialize_rabbitmq

# setup rabbitmq agent services
cd /usr/lib/systemd/system
ln -s /usr/local/fd/fairdata-ida-v3/agents/services/rabbitmq-metadata-agent.service rabbitmq-metadata-agent.service
ln -s /usr/local/fd/fairdata-ida-v3/agents/services/rabbitmq-replication-agent.service rabbitmq-replication-agent.service
