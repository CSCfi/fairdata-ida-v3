...
if [ "$DEBUG" = "true" ]; then
    ...
    echo "LOG:                          $LOG"
    echo "ERR:                          $ERR"
    echo "AUDIT_ACTIVE_DAYS_AFTER:      $AUDIT_ACTIVE_DAYS_AFTER"
    echo "AUDIT_ACTIVE_DAYS_BEFORE:     $AUDIT_ACTIVE_DAYS_BEFORE"
    echo ""
fi
...
function format_token_list_as_json() {
    local LINES="$1"
    local PAD="${2:-4}" # Default 4
...
function format_field_list_as_json() {
    local LINES="$1"
    local PAD="${2:-4}" # Default 4
    local INDENT=$(printf '%*s' "$PAD" '')
    if [ "$LINES" ]; then
        echo "$LINES" | \
            ...
            sed -e 's/:\s\s*/": "/g' | \
            sed -e "s/^/$INDENT/" | \
            # Unquote null and boolean values 
            sed -E 's/": "(null|true|false)"/": \1/g' | 
            # Unquote numeric values and convert to JSON compatible representation...
