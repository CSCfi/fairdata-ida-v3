...
class SecurityMiddleware extends Middleware {
	...
	public function beforeController($controller, $methodName) {
		...
		if ($this->isInvalidCSRFRequired($reflectionMethod)) {
			 ...
			 */
            // IDA MODIFICATION START
            // Outgoing logout requests to the SSO are OK
            $requestUrl = $this->request->getRequestUri();
            logger('ida')->debug('SecurityMiddleware beforeController: Request URL: ' . $requestUrl);
            if (str_starts_with($requestUrl, '/logout?requesttoken=')) {
                return;
            }
            // IDA MODIFICATION END
