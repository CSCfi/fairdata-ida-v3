...
class Session implements IUserSession, Emitter {
	...
	protected function validateSession() {
		...
		// IDA MODIFICATION START
        // If there is a valid SSO session cookie, the session is valid
		try {
		    if ($this->getSSOLoginDetails() !== null) {
                return true;
            }
		} catch (\Exception $ex) {
            logger('ida')->error('Session.php: validateSession: ' . $ex->getMessage());
        }
		// IDA MODIFICATION END
	...
	public function completeLogin(IUser $user, array $loginDetails, $regenerateSessionId = true) {
		...
		if ($this->isLoggedIn()) {
			...
			logger('ida')->debug('Session.php: completeLogin:' . ' uid=' . $user->getUID());

			$lang = 'en';
			$locale = 'en_US';

            $hostname = $_SERVER['SERVER_NAME'] ?? $_SERVER['HTTP_HOST'] ?? getenv('HOSTNAME') ?? (($tmp = gethostname()) ? gethostbyname($tmp) : null);
            if ($hostname) {
			    $domain = substr($hostname, strpos($hostname, '.') + 1);
			    $prefix = preg_replace('/[^a-zA-Z0-9]/', '_', $domain);
			    $cookie = $prefix . '_fd_sso_session';
	            if (array_key_exists($cookie, $_COOKIE)) {
                    $key = \OC::$server->get(IConfig::class)->getSystemValueString('SSO_KEY');
				    try {
	...
	public function isTokenPassword($password) {
		try {
			...
			// IDA MODIFICATION START
		    // This is just noise in the logs, even when debugging, so disable this
            /*
    ...
    // IDA MODIFICATION START
    /**
     * Retrieves SSO session details, if there is a valid SSO session cookie
     */
	private function getSSOLoginDetails() {
        ...
        if ($hostname) {
		    ...
		    if (isset($_COOKIE[$cookie])) {
                ...
                try {
                    $encoded_session = $_COOKIE[$cookie];
                    $key = \OC::$server->get(IConfig::class)->getSystemValueString('SSO_KEY');
                    $session = JWT::decode($encoded_session, $key, array('HS256'));
                } catch (\Exception $e) {
                    logger('ida')->error('Session.php: getSSOLoginDetails: JWT decode failed: ' . $e->getMessage());
                    $session = null;
                }

				try {
			        if ($session
				        && $session->id === $_COOKIE[$prefix . '_fd_sso_session_id']
                        ...
                        // $uid = $session->fairdata_user->id;
		                // logger('ida')->debug('Session.php: getSSOLoginDetails: ssoSessionId=' . $ssoSessionId . ' uid=' . $uid);

                        return $session;
                    }
                } catch (\Exception $e) { 
                    logger('ida')->error('Session.php: getSSOLoginDetails: ' . $e->getMessage());
                    $session = null;
                }
            }
        }
	...
	/**
	 * Tries to login the user with SSO session cookies
	 *
	 * @return boolean
	 */
	public function trySSOLogin($request) {

        $session = $this->getSSOLoginDetails();
        ...
        if ($session) {
            ...
            logger('ida')->debug('Session.php: trySSOLogin: ssoSessionId=' . $ssoSessionId . ' uid=' . $uid);
            ...
            logger('ida')->debug('Session.php: trySSOLogin: SSO session token successfully created');

            return $this->completeLogin(
                $user,
                [ 
                    'loginName' => $uid,
                    'password' => bin2hex(random_bytes(4)) // Random 8 character password, never used elsewhere
                ],
                false
            );
        }
