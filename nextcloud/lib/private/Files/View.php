...
class View {
	...
	protected string|null $idaUser = null;
	protected string $idaMode = 'API';
	...
	public function mkdir($path) {
        ...
        if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
            try {
                \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('add', $path, null, $this->idaUser, $this->idaMode, "View::mkdir");
            } catch (\Throwable $e) { 
                $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
            }
        }
		return $result;
	...
	public function rmdir($path) {
		...
		if ($mount->getInternalPath($absolutePath) === '') {
            ...
            if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $path, null, $this->idaUser, $this->idaMode, "View::rmdir::1");
                } catch (\Throwable $e) { 
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
        ...
        if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
            try {
                \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $path, null, $this->idaUser, $this->idaMode, "View::rmdir::2");
            } catch (\Throwable $e) { 
                $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
            }
        }
		...
		if (is_resource($data)) { //not having to deal with streams in file_put_contents makes life easier
			...
			if (Filesystem::isValidPath($path)
				...
				if ($target) {
                    ...
                    if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                        try {
                            \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('add', $path, null, $this->idaUser, $this->idaMode, "View::file_put_contents::1");
                        } catch (\Throwable $e) { 
                            $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                        }
                    }
            ...
            if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation($dataChange, $path, null, $this->idaUser, $this->idaMode, "View::file_put_contents::2");
                } catch (\Throwable $e) { 
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
	...
	public function unlink($path) {
		...
		if ($mount->getInternalPath($absolutePath) === '') {
            ...
            if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $path, null, $this->idaUser, $this->idaMode, "View::unlink::1");
                } catch (\Throwable $e) { 
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
		...
		if (!$result && !$this->file_exists($path)) { //clear ghost files from the cache on delete
            ...
            if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $path, null, $this->idaUser, $this->idaMode, "View::unlink::2");
                } catch (\Throwable $e) { 
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
            ...
            if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $path, null, $this->idaUser, $this->idaMode, "View::unlink::3");
                } catch (\Throwable $e) { 
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
	...
	public function deleteAll($directory) {
        ...
        if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
            try {
                \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('delete', $directory, null, $this->idaUser, $this->idaMode, "View::deleteAll");
            } catch (\Throwable $e) { 
                $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
            }
        }
	...
	public function rename($source, $target, array $options = []) {
        ...
        if (class_exists('\OCA\IDA\Controller\DataChangeController')) {
            try {
            \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('rename', $source, $target, $this->idaUser, $this->idaMode, "View::rename");
            } catch (\Throwable $e) { 
                $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
            }
        }
	...
	public function copy($source, $target, $preserveMtime = false) {
		...
		if (
            ...
            // IDA MODIFICATION START
            // Fix permissions bug when copying from frozen area by ensuring the copied target in the
            // user's own storage has full permissions. If the user was able to successfully copy the
            // data, then they should have full permissions wherever the target is. Additionally, ensure
            // that checksums already stored in the file cache are copied to the new file cache records
            // for all copied files.
            if ($run) {
                try {
                    $this->logger->debug('Fixing permissions and checksums after copy to target ' . $absolutePath2, ['app' => 'core']);
                    $cache1 = $storage1->getCache();
                    $cache2 = $storage2->getCache();
                    $permissions = \OCP\Constants::PERMISSION_ALL;
                    $this->logger->debug('Fixing permissions for ' . $internalPath2, ['app' => 'core']);
                    $cache2->put($internalPath2, ['permissions' => $permissions]);
                    if ($storage2->is_file($internalPath2)) {
                        $this->logger->debug('Retrieving checksum for ' . $internalPath2, ['app' => 'core']);
                        $fileInfo1 = $cache1->get($internalPath1);
                        $checksum = $fileInfo1 ? $fileInfo1['checksum'] : null;
                        if ($checksum !== null) {
                            $this->logger->debug('Copying checksum ' . $checksum . ' for ' . $internalPath2, ['app' => 'core']);
                            $cache2->put($internalPath2, ['checksum' => $checksum]);
                        }
                    } else if ($storage2->is_dir($internalPath2)) {
                        $this->fixPermissionsAndChecksums($storage1, $storage2, $cache1, $cache2, $internalPath1, $internalPath2, $permissions);
                    }
                } catch (\Throwable $e) {
                    $this->logger->warning('Could not fix permissions and/or checksum after copy to target ' . $absolutePath2 . ': ' . $e->getMessage(), ['app' => 'core']);
                }
                // Record data change
                try {
                    \OCA\IDA\Controller\DataChangeController::processNextcloudOperation('copy', $source, $target, $this->idaUser, $this->idaMode, "View::copy");
                } catch (\Exception $e) {
                    $this->logger->error('Error encountered trying to record data change: ' . $e->getMessage() , ['app' => 'ida']);
                }
            }
    ...
    // that checksums already stored in the file cache are copied to the new file cache records
    // for all copied files.
    private function fixPermissionsAndChecksums(
        \OC\Files\Storage\Storage $storage1,
        \OC\Files\Storage\Storage $storage2,
        \OCP\Files\Cache\ICache $cache1,
        \OCP\Files\Cache\ICache $cache2,
        string $internalPath1,
        string $internalPath2,
        int $permissions): void {

        $scanner = $storage2->getScanner();

        // Ensure this folder is scanned (helps populate cache before we walk)
        $scanner->scan($internalPath2);

        $dir = $storage2->opendir($internalPath2);
        ...
        while (($entry = readdir($dir)) !== false) {
            ...
            $childPath2 = rtrim($internalPath2, '/') . '/' . $entry;

            try {
                $type = $storage2->filetype($childPath2);
            } catch (\Throwable $e) {
                continue;
            }

            // Scan this child explicitly
            $scanner->scan($childPath2);

            // Set permissions of child
            $this->logger->debug('Fixing permissions for ' . $childPath2, ['app' => 'core']);
            $cache2->put($childPath2, ['permissions' => $permissions]);

            if ($type === 'file') {
                // Derive $childPath1 equivalent in cache1 by replacing $internalPath2 prefix of $childPath2 with $internalPath1
                $relativePath = substr($childPath2, strlen($internalPath2));
                // Ensure leading slash doesn't get doubled
                $childPath1 = rtrim($internalPath1, '/') . '/' . ltrim($relativePath, '/');
                // Get checksum, if any, for $childPath1
                $this->logger->debug('Retrieving checksum for ' . $childPath1, ['app' => 'core']);
                $fileInfo1 = $cache1->get($childPath1);
                $checksum = $fileInfo1 ? $fileInfo1['checksum'] : null;
                // Copy checksum, if any, from $childPath1 to $childPath2
                if ($checksum !== null) {
                    $this->logger->debug('Copying checksum for ' . $childPath2, ['app' => 'core']);
                    $cache2->put($childPath2, ['checksum' => $checksum]);
                }
            } else if ($type === 'dir') {
                $this->fixPermissionsAndChecksums($storage1, $storage2, $cache1, $cache2, $internalPath1, $childPath2, $permissions);
            }
        }
