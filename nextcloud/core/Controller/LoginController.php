...
class LoginController extends Controller {
	...
	public function logout() {
		...
		logger('ida')->debug('LoginController.php: logout initiated');
		...
		logger('ida')->debug('LoginController.php: redirect_url: ' . $redirect_url);

		// If login was via SSO, remove all SSO session cookies, which effectively logs out of the SSO session

        $hostname = $_SERVER['SERVER_NAME'] ?? $_SERVER['HTTP_HOST'] ?? getenv('HOSTNAME') ?? (($tmp = gethostname()) ? gethostbyname($tmp) : null);

		logger('ida')->debug('LoginController.php: hostname: ' . $hostname);
        ...
        if ($hostname) {
		    ...
		    logger('ida')->debug('LoginController.php: cookie: ' . $cookie);

            if (isset($_COOKIE[$cookie])) {
		        logger('ida')->debug('LoginController.php: logout: active SSO session cookie found');
